import { RiEditLine, RiTeamLine, RiUserAddLine, RiUserLine } from "@remixicon/react";
				...
				<div className="flex h-full w-full flex-col justify-between">
					<div className="h-[calc(100%-60px)] w-full space-y-6">
						...
						<div className="h-[214px] w-full">
							...
							<div className="-mt-[60px] flex w-full items-baseline justify-between px-5">
								<div className="flex items-baseline gap-x-3">
									...
									<div>
										<p className="font-medium capitalize">{adminName}</p>
								...
								<div
									className={cn(
										...
										admin.is_blocked ? "bg-red-100 text-red-600" : "bg-green-100 text-green-600"
									)}>
									{admin.is_blocked ? "Blocked" : "Active"}
								</div>
							</div>
						</div>
						<div className="h-[calc(100%-240px)] w-full space-y-6">
							...
							<div className="h-[calc(100%-96px)] w-full flex-1 overflow-y-auto">
								<Profile tab={tab} admin={admin} />
								<Referrals tab={tab} admin={admin} />
								<Wards tab={tab} admin={admin} />
							</div>
						</div>
					</div>
